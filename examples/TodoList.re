...
let todoListFilterState =
  Recoil.atom({key: "todoListFilterState", default: ShowAll});
...
let todoListStatsState =
  Recoil.selector({
    ...
    get: ({get}) => {
      ...
      let totalCompletedNum =
        todoList->Array.keep(item => item.isComplete)->Array.length;
      let totalUncompletedNum = totalNum - totalCompletedNum;
      let percentCompleted =
        totalNum === 0
          ? 0.0 : totalCompletedNum->Float.fromInt /. totalNum->Float.fromInt;
  ...
  let make = (~item) => {
    ...
    let editItemText = event => {
      ...
      | Some(index) =>
        let value = event->ReactEvent.Form.target##value;
        let newList =
          replaceItemAtIndex(todoList, index, {...item, text: value});
...
module TodoListStats = {
  ...
  let make = () => {
    ...
    <ul>
      <li> {j|Total items: $totalNum|j}->React.string </li>
      <li> {j|Items completed: $totalCompletedNum|j}->React.string </li>
      <li> {j|Items not completed: $totalUncompletedNum|j}->React.string </li>
      <li>
        {j|Percent completed: $(formattedPercentCompleted)%|j}->React.string
      </li>
    </ul>;
  };
};
...
ReactDOMRe.renderToElementWithId(
  <Recoil.RecoilRoot> <TodoList /> </Recoil.RecoilRoot>,
  "root",
);
